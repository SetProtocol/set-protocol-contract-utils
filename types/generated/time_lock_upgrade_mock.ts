...
import { TimeLockUpgradeMock as ContractArtifacts } from '../../artifacts/ts/TimeLockUpgradeMock';
...
export class TimeLockUpgradeMockContract extends BaseContract {
  ...
  > {
      const self = this as TimeLockUpgradeMockContract;
  ...
  static async deployed(web3: Web3, defaults: Tx): Promise<TimeLockUpgradeMockContract> {
    const web3Utils = new Web3Utils(web3);
    const currentNetwork = await web3Utils.getNetworkIdAsync();
    const { abi, networks }: { abi: any[]; networks: any } = ContractArtifacts;
    const web3ContractInstance = new web3.eth.Contract(abi, networks[currentNetwork].address);

    return new TimeLockUpgradeMockContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('TimeLockUpgradeMock', currentNetwork),
      );
    }
  }
