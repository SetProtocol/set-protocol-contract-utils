...
import { CompoundUtilsMock as ContractArtifacts } from '../../artifacts/ts/CompoundUtilsMock';
...
export class CompoundUtilsMockContract extends BaseContract {
  public testConvertCTokenToUnderlying = {
    async callAsync(
      _cTokenUnits: BigNumber,
      _exchangeRate: BigNumber,
      defaultBlock?: any,
    ): Promise<BigNumber
  > {
      const self = this as CompoundUtilsMockContract;
      const result = await promisify<BigNumber
  >(
        self.web3ContractInstance.methods.testConvertCTokenToUnderlying(
  ...
  static async deployed(web3: Web3, defaults: Tx): Promise<CompoundUtilsMockContract> {
    const web3Utils = new Web3Utils(web3);
    const currentNetwork = await web3Utils.getNetworkIdAsync();
    const { abi, networks }: { abi: any[]; networks: any } = ContractArtifacts;
    const web3ContractInstance = new web3.eth.Contract(abi, networks[currentNetwork].address);

    return new CompoundUtilsMockContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('CompoundUtilsMock', currentNetwork),
      );
    }
  }
