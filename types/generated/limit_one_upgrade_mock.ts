...
import { LimitOneUpgradeMock as ContractArtifacts } from '../../artifacts/ts/LimitOneUpgradeMock';
...
export class LimitOneUpgradeMockContract extends BaseContract {
  ...
  > {
      const self = this as LimitOneUpgradeMockContract;
  ...
  > {
      ...
      const result = await promisify<string
  >(
        self.web3ContractInstance.methods.upgradeIdentifier(
          this.formatABIDataItem(
          	'address',
          	[
          	],index_0),
  ...
  public testLimitOneUpgrade = {
    async sendTransactionAsync(
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(
        txData,
        self.testLimitOneUpgrade.estimateGasAsync.bind(
          self,
          this.formatABIDataItem(
          	'address',
          	[
      ...
      const txHash = await promisify<string>(
        self.web3ContractInstance.methods.testLimitOneUpgrade(
          this.formatABIDataItem(
          	'address',
          	[
          	],_upgradeAddress),
    ...
    getABIEncodedTransactionData(
      ...
      const self = this as LimitOneUpgradeMockContract;
      const abiEncodedTransactionData = self.web3ContractInstance.methods.testLimitOneUpgrade(
        this.formatABIDataItem(
        	'address',
        	[
        	],_upgradeAddress),
  ...
  static async deployed(web3: Web3, defaults: Tx): Promise<LimitOneUpgradeMockContract> {
    const web3Utils = new Web3Utils(web3);
    const currentNetwork = await web3Utils.getNetworkIdAsync();
    const { abi, networks }: { abi: any[]; networks: any } = ContractArtifacts;
    const web3ContractInstance = new web3.eth.Contract(abi, networks[currentNetwork].address);

    return new LimitOneUpgradeMockContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('LimitOneUpgradeMock', currentNetwork),
      );
    }
  }
