...
import { LimitOneUpgrade as ContractArtifacts } from '../../artifacts/ts/LimitOneUpgrade';
...
export class LimitOneUpgradeContract extends BaseContract {
  ...
  > {
      const self = this as LimitOneUpgradeContract;
  ...
  static async deployed(web3: Web3, defaults: Tx): Promise<LimitOneUpgradeContract> {
    const web3Utils = new Web3Utils(web3);
    const currentNetwork = await web3Utils.getNetworkIdAsync();
    const { abi, networks }: { abi: any[]; networks: any } = ContractArtifacts;
    const web3ContractInstance = new web3.eth.Contract(abi, networks[currentNetwork].address);

    return new LimitOneUpgradeContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    defaults: Tx,
  ): Promise<LimitOneUpgradeContract> {
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('LimitOneUpgrade', currentNetwork),
      );
    }
  }
