...
import { Bytes32Library as ContractArtifacts } from '../../artifacts/ts/Bytes32Library';
...
export class Bytes32LibraryContract extends BaseContract {
  ...
  > {
      const self = this as Bytes32LibraryContract;
      const result = await promisify<string
  >(
        self.web3ContractInstance.methods.bytes32ToString(
  ...
  static async deployed(web3: Web3, defaults: Tx): Promise<Bytes32LibraryContract> {
    const web3Utils = new Web3Utils(web3);
    const currentNetwork = await web3Utils.getNetworkIdAsync();
    const { abi, networks }: { abi: any[]; networks: any } = ContractArtifacts;
    const web3ContractInstance = new web3.eth.Contract(abi, networks[currentNetwork].address);

    return new Bytes32LibraryContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    defaults: Tx,
  ): Promise<Bytes32LibraryContract> {
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('Bytes32Library', currentNetwork),
      );
    }
  }
