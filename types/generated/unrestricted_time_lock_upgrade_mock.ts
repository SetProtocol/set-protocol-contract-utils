...
import { UnrestrictedTimeLockUpgradeMock as ContractArtifacts } from '../../artifacts/ts/UnrestrictedTimeLockUpgradeMock';
...
export class UnrestrictedTimeLockUpgradeMockContract extends BaseContract {
  ...
  > {
      const self = this as UnrestrictedTimeLockUpgradeMockContract;
  ...
  > {
      ...
      const result = await promisify<BigNumber
  >(
        self.web3ContractInstance.methods.testUint(
        ).call,
        self.web3ContractInstance,
      )();
      return new BigNumber(result);
  ...
  public testTimeLockUpgrade = {
    async sendTransactionAsync(
      ...
      const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(
        txData,
        self.testTimeLockUpgrade.estimateGasAsync.bind(
          self,
          this.formatABIDataItem(
          	'uint256',
          	[
      ...
      const txHash = await promisify<string>(
        self.web3ContractInstance.methods.testTimeLockUpgrade(
          this.formatABIDataItem(
          	'uint256',
          	[
          	],_testUint),
    ...
    getABIEncodedTransactionData(
      ...
      const self = this as UnrestrictedTimeLockUpgradeMockContract;
      const abiEncodedTransactionData = self.web3ContractInstance.methods.testTimeLockUpgrade(
        this.formatABIDataItem(
        	'uint256',
        	[
        	],_testUint),
  ...
  static async deployed(web3: Web3, defaults: Tx): Promise<UnrestrictedTimeLockUpgradeMockContract> {
    const web3Utils = new Web3Utils(web3);
    const currentNetwork = await web3Utils.getNetworkIdAsync();
    const { abi, networks }: { abi: any[]; networks: any } = ContractArtifacts;
    const web3ContractInstance = new web3.eth.Contract(abi, networks[currentNetwork].address);

    return new UnrestrictedTimeLockUpgradeMockContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('UnrestrictedTimeLockUpgradeMock', currentNetwork),
      );
    }
  }
