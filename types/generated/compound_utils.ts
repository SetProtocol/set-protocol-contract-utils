...
import { CompoundUtils as ContractArtifacts } from '../../artifacts/ts/CompoundUtils';
...
export class CompoundUtilsContract extends BaseContract {
  ...
  static async deployed(web3: Web3, defaults: Tx): Promise<CompoundUtilsContract> {
    const web3Utils = new Web3Utils(web3);
    const currentNetwork = await web3Utils.getNetworkIdAsync();
    const { abi, networks }: { abi: any[]; networks: any } = ContractArtifacts;
    const web3ContractInstance = new web3.eth.Contract(abi, networks[currentNetwork].address);

    return new CompoundUtilsContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    defaults: Tx,
  ): Promise<CompoundUtilsContract> {
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('CompoundUtils', currentNetwork),
      );
    }
  }
